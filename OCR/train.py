...
def train(opt):
	""" dataset preparation """
	train_dataset = MCPDatesDataset(opt.train_data, opt, is_training=True, data_aug=True, img_w=opt.img_w, img_h=opt.img_h)
	valid_dataset = MCPDatesDataset(opt.valid_data, opt, is_training=False, data_aug=False, img_w=opt.img_w, img_h=opt.img_h)
	...
	while(True):
		...
		for batch in tqdm(train_loader):
			image_tensors = batch[0]; labels = batch[1]
