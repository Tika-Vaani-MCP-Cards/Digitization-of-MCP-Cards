...
# performs only vertical crop
class CVRandomCrop(object):
    def __init__(self, px_max=20, p=0.5):
        self.p = p
        self.px_max = px_max
    ...
    def __call__(self, img):
        ...
        if random.random() < self.p:
            # add try except block if random crop fails
            image = img.copy()
            try:
                img = np.array(img)
                h, w = img.shape[:2]
                px = random.randint(5, self.px_max)
                img = img[px:h-px, :]
                img = cv2.resize(img, (w, h), interpolation=get_interpolation())
